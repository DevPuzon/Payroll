            ...
            <div class="box-body">
              <div class="table-wrapper-scroll-y my-custom-scrollbar">
                <table id="example1" class="table table-bordered table-striped mb-0">
                  ...
                  <tbody>
                    <?php
                      $sql = "SELECT *, employees.id AS empid FROM employees LEFT JOIN position ON position.id=employees.position_id LEFT JOIN schedules ON schedules.id=employees.schedule_id  LEFT JOIN tbl_department ON tbl_department.col_departmentid=employees.department_id";
                      ...
                      while($row = $query->fetch_assoc()){
                        ?>
                          <tr>
                            ...
                            <td><?php echo $row['col_department']; ?></td>
                            <td><?php 
                            if(!$row['col_is_flexi']){
...
function getRow(id){
  $.ajax({
    ...
    success: function(response){
      ...
      $('#department_val').val(response.department_id).html(response.col_department);
