			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					...
					if($conn->query($sql)){ 
						...
						if(!$isFlexirow['col_is_flexi']){  
							...
							$sqlUpdateT = "update attendance set num_hr = '8' where id = $docid";
							$tquery = $conn->query($sqlUpdateT);  
							$output['message'] = 'Time in: '.$row['firstname'].' '.$row['lastname'].$sqlUpdateT ;
						}
					}
					else{
						$output['error'] = true;
				...
				if($query->num_rows < 1){
					...
					if($row['time_out'] != '00:00:00'){
						...
						$sql = "UPDATE attendance SET time_out = NOW(), description_out = '$desc',proofimg_out ='$proof' WHERE id = '".$row['uid']."'";
						if($conn->query($sql)){
